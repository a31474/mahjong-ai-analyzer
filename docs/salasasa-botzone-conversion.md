...
- 想要「喂本仓库引擎」→ 只能用 `converter.py`。字母约定修正后两边已一致，但上游输出**仍不能直接搬**：上游是一份**上帝视角合并** log（发牌行含各家手牌与全部花牌、他人摸牌写成 `2 CARD`），而引擎要的是**单座视角**（每人只见自己 13/14 张，他人摸牌是 `Player N Draw`）；加上上游座位是启发式猜测（§4），直接转换会同时错在视角与座位
